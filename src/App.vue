<script setup>
  ...
  const { expand, isExpanded, viewportHeight, viewportStableHeight, isVerticalSwipesEnabled } =
    useWebAppViewport();
  ...
  function toggleMainButton() {
    isMainButtonVisible.value ? hideMainButton() : showMainButton();
  }

  function toggleMainButtonProgress() {
    isMainButtonProgressVisible.value ? hideMainButtonProgress() : showMainButtonProgress(true);
  }
...
<template>
  <section>
    ...
    <button @click.prevent="sendData('Hello, World!')">Send «Hello, World!»</button>
  </section>

  <section>
    <div>
      <h4>Main Button:</h4>
      <p>isMainButtonActive: {{ isMainButtonActive }}</p>
      <p>isMainButtonVisible: {{ isMainButtonVisible }}</p>
      <p>isMainButtonProgressVisible: {{ isMainButtonProgressVisible }}</p>
      <p>mainButtonText: {{ mainButtonText }}</p>
      <p>mainButtonColor: {{ mainButtonColor }}</p>
      <p>mainButtonTextColor: {{ mainButtonTextColor }}</p>
    ...
    <div>
      ...
      <p>Viewport stable height: {{ viewportStableHeight }}</p>
      <p>Expanded: {{ isExpanded }}</p>
      <p>Vertical Swipes: {{ isVerticalSwipesEnabled }}</p>

      <button @click.prevent="expand()">Expand Viewport</button>
    ...
    <div>
      <h4>Navigation:</h4>
      <ul>
        <li><a href="?nextpage=1">Regular link</a> (opens inside webview)</li>
        <li>
          <a href="https://telegram.org/" target="_blank">target="_blank" link</a> (opens outside
          webview)
        </li>
        <li>
          <a href="javascript:window.open('https://telegram.org/');">window.open() link</a> (opens
          outside webview)
        </li>
        <li><a href="https://t.me/like">LikeBot t.me link</a> (opens inside Telegram app)</li>
        <li>
          <a href="#" @click.prevent="openTelegramLink('https://t.me/vote')">openTelegramLink()</a>
        ...
        <li>
          <a href="#" @click.prevent="switchInlineQuery('query')">switchInlineQuery('query')</a>
    ...
    <div>
      ...
      <ul>
        <li>
          Impact: <a href="#" @click.prevent="impactOccurred('heavy')">heavy</a>, &nbsp;
          <a href="#" @click.prevent="impactOccurred('light')">light</a>, &nbsp;
          <a href="#" @click.prevent="impactOccurred('medium')">medium</a>, &nbsp;
          <a href="#" @click.prevent="impactOccurred('rigid')">rigid</a>, &nbsp;
          <a href="#" @click.prevent="impactOccurred('soft')">soft</a><br /><br />
        </li>
        <li>
          Notification: <a href="#" @click.prevent="notificationOccurred('error')">error</a>, &nbsp;
          <a href="#" @click.prevent="notificationOccurred('success')">success</a>, &nbsp;
          <a href="#" @click.prevent="notificationOccurred('warning')">warning</a><br /><br />
        </li>
        <li>Selection: <a href="#" @click.prevent="selectionChanged()">changed</a><br /><br /></li>
      </ul>
    </div>
    ...
    <div>
      <h4>Theme params</h4>
      <div class="sect_row">Color scheme: {{ colorScheme }}</div>
      <div class="sect_row">
        Header:
        <select disabled>
    ...
    <div>
      ...
      <button @click.prevent="requestAccessBiometricManager">request access</button>
      <button @click.prevent="authenticateBiometricManager">authenticate</button>
      <button @click.prevent="openSettingsBiometricManager">open settings</button>
    </div>
